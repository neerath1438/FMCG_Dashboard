...
def test_model(item):
    ...
    try:
        ...
        # Minimal parameters as 'temperature' seems unsupported
...
for r in results:
    ...
    issues = []
    # Logic check for POCKY/YAN YAN
    if "POCKY" in inp and "POCKY" not in str(out.get("product_line", "")).upper():
        issues.append("Missing POCKY product line")
    if "YAN YAN" in inp and "YAN YAN" not in str(out.get("product_line", "")).upper():
